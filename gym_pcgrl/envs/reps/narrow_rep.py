...
class NarrowRepresentation(Representation):
    ...
    def reset(self, width, height, prob):
        # print(f"inside of narrow.reset()")
    ...
    def update(self, action):
        ...
        # if action == 8:
        #     print(f"We're in narrow.update(action), actions is: {action}")
        # TODO: ask Ahmed about case when action is 8 (i.e. no change) wouldn't this make line 104 true (i.e. 1) even though no tile change occurs?
        change = 0
        change += [0,1][self._map[self._y][self._x] != action-1]
        self._map[self._y][self._x] = action - 1
        # if action > 0:
        #     # if action == 8:
        #     #     print(f"[0,1][self._map[self._y][self._x] != action-1] is {[0,1][self._map[self._y][self._x] != action-1]}")
        #     change += [0,1][self._map[self._y][self._x] != action-1]
        #     self._map[self._y][self._x] = action-1
        if self._random_tile:
    ...
    def render(self, lvl_image, tile_size, border_size):
        # print(f"lvl_image is {lvl_image.show()}")
        ...
        # print(f"xgraphics is {x_graphics.show()}")
